    ...
    >
        <a-form :form="form">
            ...
            <a-form-item v-bind="formItemLayout" label="结算后总价">
                <span v-if="finalPrice===''">￥{{ totalPrice }}</span>
        ...
        methods: {
            ...
            onchange() {
                ...
                if(this.checkedList.length>0){
                    this.orderMatchCouponList.filter(item => this.checkedList.indexOf(item.id)!=-1).forEach(function (item) {
                        if(item.discount>0){
                        }else{
                            that.finalPrice= that.finalPrice-item.discountMoney
                        }
                    })
                    this.orderMatchCouponList.filter(item => this.checkedList.indexOf(item.id)!=-1).forEach(function (item) {
                        if(item.discount>0){
                            that.finalPrice=that.finalPrice*item.discount
                        }else{
